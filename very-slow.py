...
def menu(my_name,my_id):
	...
	banner()
	sg = '# 💀AUTHOR :kozhay_mam_bakrm   '
	fx = mark(sg, style='bold yellow')
	sol().print(fx)
	io = '◈───[01]─── CRACK id PUBLIC FRIENDS\n◈───[02]─── CRACK FROM MULTI ADD [ UNLIMITED ]\n◈───[03]─── CHECK CRACKING ID RESULT\n◈───[04]─── CHECKPOINT OPTIONS\n◈───[00] ─── TYPE 00 AND LOGOUT'
	oi = nel(io, style='bold yellow')
	cetak(nel(oi, title='🔥MENU-PROTOCOL🔥'))
	jh = input(x+''+p+'🔥'+x+' TYPE YOUR DESIRE NUMBER & ENTER : ')
	if jh in ['1','01']:
		dump_publik()
	elif jh in ['2','02']:
		dump_massal()
...
def dump_publik():
	...
	try:
		...
		print(x+'['+h+'•'+x+'] 💀TOTAL-DUMP💀: '+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s💀KOZHA🔥 %s/%s 🔥OK:%s 🔥CP:%s 🔥 %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
def crack2(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	print('\r%s💀KOZHYAR💀 %s/%s 💀Ok*%s 💀CP *%s 💀 %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
